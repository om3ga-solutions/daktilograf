...
This section describes how to build DAKTILOGRAFSTT binaries.

It is strongly recommended that you always use our pre-built DAKTILOGRAF STT binaries (available with every `release <https://github.com/om3gasolutions/daktilograf-3.0/releases>`_) unless you have a reason to do build them yourself.

If you would still like to build the DAKTILOGRAF STT binaries yourself, you'll need the following pre-requisites downloaded and installed:
...
If you follow these instructions, you should compile your own binaries of DAKTILOGRAF STT (built on TensorFlow using Bazel).
...
Clone DAKTILOGRAF STT source code (TensorFlow will come as a submdule):

.. code-block::

   git clone https://github.com/om3gasolutions/daktilograf-3.0.git STT
   cd STT
...
Within your TensorFlow directory, there should be a symbolic link to the DAKTILOGRAF STT ``native_client`` directory. If it is not present, create it with the follow command:
...
You can now use Bazel to build the main DAKTILOGRAF STT library, ``libstt.so``. Add ``--config=cuda`` if you want a CUDA build.
...
We only support building CTC Decoder on x86-64 architectures. However, we offer some hints on building the CTC decoder on other architectures, and you might find some help in our `GitHub Discussions <https://github.com/om3gasolutions/STT/discussions>`.
...
We do support cross-compilation. Please refer to our ``om3gasolutions/tensorflow`` fork, where we define the following ``--config`` flags:
...
Please push DAKTILOGRAF STT data to ``/sdcard/STT/``\ , including:
